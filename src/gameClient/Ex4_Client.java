 ...
 * file performs the following tasks: 0. login as a user ("999") for testing -
 * do use your ID. 1. Creates a game_service [0,23] (user "999" has stage 9, can
 * play in scenarios [0,9] not above Note: you can also choose -1 for debug
 * (allowing a 600 second game). 2. Constructs the graph from JSON String 3.
 * Gets the scenario JSON String 5. Add a set of robots // note: in general a
 * list of robots should be added 6. Starts game 7. Main loop (vary simple
 * thread) 8. move the robot along the current edge 9. direct to the next edge
 * (if on a node) 10. prints the game results (after "game over"), and write a
 * KML: Note: will NOT work on case -1 (debug).
 * 
